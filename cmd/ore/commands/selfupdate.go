...
import (
	...
	"github.com/contriboss/ore-light/internal/selfupdate"
)
...
func RunSelfUpdate(args []string, currentVersion, buildCommit string) error {
	...
	client := selfupdate.NewClient("contriboss", "ore-light")
	...
	fmt.Printf("v%s\n", latest.Version.String())
	fmt.Printf("New release found! v%s --> v%s\n", current.String(), latest.Version.String())
	...
	newTargetArch := fmt.Sprintf("ore-v%s-%s-%s.tar.gz", latest.Version.String(), runtime.GOOS, runtime.GOARCH)
	...
	if err := updater.UpdateTo(asset.BrowserDownloadURL, exe); err != nil {
